<template>
  <div id="map">
    <l-map ref="map" v-if="mapData" v-bind="{ options }">
      <l-tile-layer :url="url" :attribution="attribution" />
      <l-control-zoom position="bottomright" />

      <l-geo-json :geojson="mapData"></l-geo-json>
    </l-map>
  </div>
</template>
...
export default {
  name: 'Map',
  components: { LMap, LTileLayer, LControlZoom, LGeoJson },
  data() {
    return {
      mapData: null,
  ...
  mounted() {
    // ApiService.get('https://rawgit.com/gregoiredavid/france-geojson/master/regions/pays-de-la-loire/communes-pays-de-la-loire.geojson').then((response) => {
    ApiService.get('map-subunitati/?format=json').then((response) => {
